...
//            if (db.insereixPokemon(p.getNombre(), p.getDescripcion(), p.getTipoDual(), p.getTipo()) == -1) {
...
//                Toast.LENGTH_SHORT).show();
        ...
        while(!c.isAfterLast()){
            if(c.getString(3).equals("0")){
                ...
                Pokemon p = new Pokemon(c.getString(0), c.getString(1), c.getString(2), tipo);
                pokemons.add(p);
            }else{
                Tipo tipo1 = new Tipo();
                Tipo tipo2 = new Tipo();
                ...
                Pokemon p = new Pokemon(c.getString(0), c.getString(1),c.getString(2), new TipoDual(tipo1, tipo2));
                pokemons.add(p);
            }
            c.moveToNext();
        }
        ...
        PokemonsArray pokemonsArray = new PokemonsArray(this, R.layout.pokemon_llista, pokemons);
